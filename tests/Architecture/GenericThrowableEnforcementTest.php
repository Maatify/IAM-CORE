...
final class GenericThrowableEnforcementTest extends TestCase
{
    /**
     * Short names forbidden in "throw new"
     *
     * @var array<string, true>
     */
    private const FORBIDDEN_SHORT = [
        'Exception'        => true,
        'RuntimeException' => true,
    ];

    /**
     * Fully-qualified names forbidden in "throw new"
     *
     * @var array<string, true>
     */
    ...
    public function test_it_forbids_throwing_generic_php_exceptions(): void
    {
        ...
        $this->assertSame(
            [],
            $violations,
            "Forbidden generic PHP exception throwing detected:\n- " . implode("\n- ", $violations)
        );
    }
    ...
    private function findForbiddenThrows(string $code): array
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (
                isset(self::FORBIDDEN_SHORT[$short]) ||
                isset(self::FORBIDDEN_FQCN[$className])
            ) {
                $found[] = $className;
            }
        }
